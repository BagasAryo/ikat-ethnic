...
@section('title', 'Products | Ikat Ethnic')
            ...
            <div class="flex flex-col gap-3">
              @foreach ($categories as $category)
                @if ($category->products_count > 0 || in_array($category->id, (array) request('categories', [])))
                  <label
                    class="flex items-center gap-3 cursor-pointer group text-sm text-ink hover:text-gold-lt transition-colors">
        ...
        <div class="hidden lg:flex items-center justify-between mb-8 pb-4 border-b border-white/5">
          <span class="text-xs text-muted tracking-wide">
            Showing <span class="text-ink font-medium">{{ $products->firstItem() ?? 0 }}</span>–<span
              ...
              class="text-ink font-medium">{{ $products->total() }}</span> masterpieces
          </span>
        ...
        @if ($products->isEmpty())
          ...
          <div class="text-center py-24 bg-surface/20 border border-dashed border-surface2 rounded-sm px-4">
            ...
            <p class="text-muted text-sm max-w-sm mx-auto mb-8 font-light leading-relaxed">
              We couldn't find any items matching your selected criteria. Try adjusting your search keyword or clearing
              the filters.
            </p>
            <a href="{{ route('products') }}"
