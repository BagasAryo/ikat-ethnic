...
<head>
  ...
  <title>My Orders | Ikat Ethnic</title>
        ...
        @if($orders->isEmpty())
          ...
          <div class="bg-surface border border-surface2 p-12 text-center flex flex-col items-center justify-center gap-6">
            ...
            <div>
              ...
              <p class="text-muted text-sm mt-1 max-w-sm">You haven't placed any orders yet. Start exploring our premium collections!</p>
            </div>
          ...
          <div class="flex flex-col gap-6">
            @foreach($orders as $order)
              <div class="bg-surface border border-surface2 rounded-sm overflow-hidden flex flex-col">
                ...
                <!-- Order Header -->
                <div class="bg-surface2/40 px-6 py-4 border-b border-surface2 flex flex-wrap justify-between items-center gap-4">
                  <div>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    @endphp
                    <span class="px-3 py-1 border text-xs font-medium uppercase tracking-wider rounded-sm {{ $statusClass }}">
                      {{ $order->status }}
                    </span>
                    <a href="{{ route('orders.show', $order->id) }}" class="px-3 py-1 border border-surface2 hover:border-gold text-muted hover:text-gold text-xs font-medium uppercase tracking-wider rounded-sm transition-colors">
                      Details
                    </a>
                  </div>
                </div>
                  ...
                  @foreach($order->orderItems as $item)
                    <div class="py-4 flex gap-4 items-center">
                      ...
                      <div class="text-right shrink-0">
                        <p class="text-white text-sm font-medium">Rp{{ number_format($item->subtotal, 0, ',', '.') }}</p>
                ...
                <!-- Order Footer -->
                <div class="bg-bg/40 px-6 py-4 border-t border-surface2 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <!-- Shipping Info -->
                  ...
                  <div class="text-right flex flex-col items-end gap-3 w-full md:w-auto shrink-0">
                    <div class="text-xs text-muted flex flex-col gap-1 w-full sm:w-48">
                      ...
                      <div class="flex justify-between text-sm text-white font-medium border-t border-surface2/30 pt-1.5 mt-1">
                        <span>Total:</span>
                        <span class="text-gold font-semibold">Rp{{ number_format($order->total_amount, 0, ',', '.') }}</span>
                      </div>
                    </div>
                    ...
                    @if(strtolower($order->status) === 'pending' && $order->payment && $order->payment->snap_token && strtolower($order->payment->status) === 'pending')
                      <button type="button" onclick="payOrder('{{ $order->payment->snap_token }}', '{{ $order->id }}')"
