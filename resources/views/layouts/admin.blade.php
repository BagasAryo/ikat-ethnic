...
<head>
    ...
    <style>
        ...
        #admin-sidebar::-webkit-scrollbar-thumb { background: #2a2a2a; border-radius: 2px; }
    </style>
</head>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside id="admin-sidebar"
        ...
        <nav class="flex-1 px-3 py-6 space-y-1">
            ...
            <a href="{{ route('admin.orders.index') }}"
               ...
               class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-sm text-sm font-medium transition-all duration-200
                ...
                <span class="sidebar-text ml-auto bg-gold text-bg text-[10px] font-bold px-1.5 py-0.5 rounded-full leading-none">
                    {{ $pendingOrdersCount }}
                </span>
                @endif
            </a>
        ...
        <div class="shrink-0 border-t border-white/5 px-4 py-4">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-surface2 border border-white/10 flex items-center justify-center shrink-0">
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        {{-- Top Navbar --}}
        <header class="shrink-0 flex items-center justify-between h-14 px-6 border-b border-white/5 bg-surface/50 backdrop-blur-sm z-20">

            {{-- Left: Toggle + Breadcrumb --}}
            <div class="flex items-center gap-4">
                <button id="sidebar-toggle"
                        class="text-muted hover:text-ink transition-colors -ml-1"
            ...
            <div class="flex items-center gap-4">
                {{-- Search --}}
                <div class="hidden md:flex items-center gap-2 bg-surface2 border border-white/5 rounded-sm px-3 py-1.5 text-sm">
                    <i data-feather="search" class="w-3.5 h-3.5 text-faint"></i>
                    <input type="text" placeholder="Cari..." class="bg-transparent text-ink text-xs outline-none placeholder:text-faint w-40">
                </div>
