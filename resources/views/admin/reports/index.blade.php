...
@section('content')
  ...
  {{-- ── Filter Form ──────────────────────────────────────────────── --}}
  <div class="bg-surface border border-white/5 rounded-sm p-5 mb-6">
    <form method="GET" action="{{ route('admin.reports.index') }}"
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <input type="date" id="date_from" name="date_from"
          ...
          class="bg-surface2 border border-white/10 rounded-sm text-ink text-xs px-3 py-2 outline-none
        ...
        <div class="flex items-center gap-1.5 flex-wrap">
          @foreach ([
            ...
            <a href="{{ route('admin.reports.index', ['date_from' => $preset['from'], 'date_to' => $preset['to']]) }}"
              class="text-[10px] px-2.5 py-1.5 rounded-sm border transition-all duration-150
                     {{ request('date_from') === $preset['from'] && request('date_to') === $preset['to']
                          ? 'bg-gold text-bg border-gold font-semibold'
                          : 'bg-surface2 text-muted border-white/10 hover:border-gold/30 hover:text-gold' }}">
              {{ $preset['label'] }}
            </a>
          @endforeach
        </div>
      ...
      {{-- Submit --}}
      <button type="submit"
        class="flex items-center gap-2 bg-gold text-bg text-xs font-semibold px-4 py-2 rounded-sm
               hover:bg-gold-lt transition-colors duration-150 shrink-0">
  ...
  <div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-surface border border-white/5 rounded-sm p-5 hover:border-gold/20 transition-colors duration-300">
      ...
      <p class="text-2xl font-semibold text-ink tracking-tight">{{ $totalOrders }}</p>
    ...
    <div class="bg-surface border border-white/5 rounded-sm p-5 hover:border-gold/20 transition-colors duration-300">
      ...
      <p class="text-muted text-xs mt-1 uppercase tracking-wider">Total Revenue</p>
    </div>
    ...
    <div class="bg-surface border border-white/5 rounded-sm p-5 hover:border-gold/20 transition-colors duration-300">
      ...
      <p class="text-2xl font-semibold text-ink tracking-tight">{{ $totalItemsSold }}</p>
      <p class="text-muted text-xs mt-1 uppercase tracking-wider">Item Terjual</p>
    </div>
    ...
    <div class="bg-surface border border-white/5 rounded-sm p-5 hover:border-gold/20 transition-colors duration-300">
      ...
      <p class="text-2xl font-semibold text-ink tracking-tight">{{ $totalNewUsers }}</p>
      <p class="text-muted text-xs mt-1 uppercase tracking-wider">User Baru</p>
    </div>
    ...
    <div class="bg-surface border border-white/5 rounded-sm p-6">
      ...
      @php
        $statusMap = [
          ...
          'Cancelled'  => ['label' => 'Dibatalkan', 'color' => 'bg-faint',     'text' => 'text-faint',     'border' => 'border-white/10',     'bg' => 'bg-surface2'],
        ];
      @endphp

      <div class="space-y-3">
        @forelse ($statusBreakdown as $row)
          @php $map = $statusMap[$row->status] ?? ['label' => $row->status, 'color' => 'bg-faint', 'text' => 'text-faint', 'border' => 'border-white/10', 'bg' => 'bg-surface2']; @endphp
              ...
              <tr class="hover:bg-surface2/40 transition-colors">
                ...
                <td class="px-6 py-3.5">
                  <div class="flex items-center gap-2.5">
                    ...
                    @else
                      <div class="w-8 h-8 rounded-sm bg-surface2 border border-white/10 flex items-center justify-center shrink-0">
