    ...
    <div class="xl:col-span-1 space-y-4">
      ...
      <div class="bg-surface border border-black/10 rounded-sm p-6">
        ...
        <div class="flex flex-col items-center text-center mb-6">
          ...
          <p class="text-muted text-xs mt-0.5">{{ $user->email }}</p>
        </div>
        ...
        <div class="space-y-4 text-xs">
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-muted text-[10px] uppercase tracking-wider mb-0.5">No. HP</p>
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-muted text-[10px] uppercase tracking-wider mb-0.5">Alamat</p>
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-muted text-[10px] uppercase tracking-wider mb-0.5">Bergabung</p>
      ...
      <div class="bg-surface border border-black/10 rounded-sm p-5">
        <p class="text-muted text-[10px] uppercase tracking-[0.15em] font-semibold mb-4">Statistik</p>
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-surface2 rounded-sm p-3 text-center">
            <p class="text-sm font-semibold text-ink">{{ $user->orders_count }}</p>
            <p class="text-muted text-[10px] uppercase tracking-wider mt-0.5">Total Order</p>
          </div>
          <div class="bg-surface2 rounded-sm p-3 text-center">
            <p class="font-semibold text-ink text-md">Rp{{ number_format($totalSpent, 0, ',', '.') }}</p>
            <p class="text-muted text-[10px] uppercase tracking-wider mt-0.5">Total Belanja</p>
          </div>
        </div>
      </div>
          ...
          @forelse ($orders as $order)
            ...
            <div class="px-5 py-4 {{ !$loop->last ? 'border-b border-black/10' : '' }}">
              <div class="flex items-center justify-between gap-2 mb-2">
                <span class="text-ink text-xs font-mono font-medium truncate">{{ $order->order_number }}</span>
                <span
              ...
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-muted text-[10px]">{{ $order->created_at->isoFormat('D MMMM Y') }}</p>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-black/10">
                <th class="text-left px-4 py-4 text-ink text-[10px] font-semibold uppercase tracking-[0.15em]">Order ID
                </th>
                <th class="text-left px-4 py-4 text-ink text-[10px] font-semibold uppercase tracking-[0.15em]">Tanggal
                </th>
                <th class="text-left px-4 py-4 text-ink text-[10px] font-semibold uppercase tracking-[0.15em]">Total
                </th>
                <th class="text-left px-4 py-4 text-ink text-[10px] font-semibold uppercase tracking-[0.15em]">Status
                </th>
                <th class="text-left px-4 py-4 text-ink text-[10px] font-semibold uppercase tracking-[0.15em]">Aksi</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-white/5">
              @forelse ($orders as $order)
                ...
                <tr class="group hover:bg-surface2/50 transition-colors duration-150">
                  <td class="px-4 py-4">
                    <span class="text-muted text-xs font-mono font-medium">{{ $order->order_number }}</span>
                  </td>
                  <td class="px-4 py-4">
